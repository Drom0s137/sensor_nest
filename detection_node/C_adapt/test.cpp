...
string formatDetectionsToJSON(int frameNum, const vector<json>& detectionsList, const string& imageBase64) {
    ...
    j["image"] = imageBase64; // base64-encoded JPEG of the processed frame
    return j.dump();
}

int main(int argc, char** argv) {
    // Usage: object_detection_zmq <input_source> [output_video_optional]
    // input_source: either a file path or "camera" (or "0") for live feed.
    ...
    VideoCapture cap;
    // Choose live camera feed if "camera" or "0" is passed.
    if (inputSource == "camera" || inputSource == "0") {
        // Using a GStreamer pipeline to force MJPG at 1280x960.
    ...
    if (fps <= 0) fps = 30.0; // default FPS if not reported.
    ...
    // Load the pre-trained MobileNet SSD ONNX model.
    // Download an ONNX model from the ONNX Model Zoo. For example, you can try:
    // https://github.com/onnx/models/tree/main/vision/object_detection_segmentation/ssd
    // and place the model as "model_zoo/mobilenet_ssd.onnx".
    string onnxModel = "model_zoo/ssd_mobilenet_v1_10.onnx";
    Net net = readNetFromONNX(onnxModel);
    if (net.empty()) {
        cerr << "Error: Could not load the network from the ONNX model file: " << onnxModel << endl;
        return -1;
    }
    
    #ifdef DNN_BACKEND_TENSORRT
    net.setPreferableBackend(DNN_BACKEND_TENSORRT);
    ...
    while (cap.read(frame)) {
        ...
        // Note: Many ONNX SSD models still expect a 300x300 input size.
